...
export class GlCodesComponent implements OnInit
{
  gridData: GLCodesDto[] = [];
  gridView: GLCodesDto[] = []
  ...
  {
    this.GLCodesGridCols = this.gridDataService.getColumnDefs(GridType.GLCodes, this.GeneralService.permissions['GL Codes']);
  }
  ...
  ngOnInit(): void
  {
    ...
    this.getAllGlCodes(currentPage, pageSize)
    this.actionCellService.primaryClicked$.pipe(takeUntil(this.isDestroyed$)).subscribe((data) =>
    {
      if (data.gridName === GridType.GLCodes)
      {
        this.onEditClick(data.rowData.glCode)
      }
    })
    this.actionCellService.secondaryClicked$.pipe(takeUntil(this.isDestroyed$)).subscribe((data) =>
    {
      if (data.gridName === GridType.GLCodes)
      {
        this.removeHandler(data.rowData.glCode)
      }
    })
  ...
  private searchHandler()
  {
    ...
    ).subscribe((term) =>
    {
      ...
      this.getAllGlCodes(1, this.pagination.pageSize);
    });
  }
  ...
  getAllGlCodes(currentPage: number, pageSize: number)
  {
    ...
    this.tableDataService.getTableDataWithPagination('GLCodes/GetAllGLCodes', payload)
      .pipe(first(), finalize(() => this.fetchingData = false))
      .subscribe({
        next: (res: GLCodesDtoResponse) =>
        {
          if (res)
          {
    ...
    this.confirmationDialogService.confirm()
      .then((confirmed) =>
      {
        if (confirmed)
        {
          ...
          const payload = { glCode }
          this.tableDataService.getTableData('GLCodes/DeleteGLCode', { delete: 1, ...payload })
            .pipe(first(), finalize(() => this.fetchingData = false))
            .subscribe({
              next: (res) =>
              {
                if (res && res.status === '200')
                {
                  ...
                  this.getAllGlCodes(this.pagination.currentPage, this.pagination.pageSize)
                } else
                {
  ...
  exportToCSV(): void
  {
    this.gridDataService.exportToCSV(this.gridApi, GridType.GLCodes);
  }
}
