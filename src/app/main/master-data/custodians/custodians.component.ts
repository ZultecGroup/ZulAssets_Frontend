...
export class CustodiansComponent implements OnInit {
  gridData: CustodiansDto[] = [];
  gridView: CustodiansDto[] = [];
  fetchingData: boolean = false;
  sendingRequest: boolean = false;
  searchText: string = '';
  searchSubject = new Subject<string>();
  custodiansGridCols: ColDef[] = [];
  ...
  {
    this.custodiansGridCols = this.gridDataService.getColumnDefs(GridType.Custodians, this.GeneralService.permissions['Custodians']);
  }
  ...
  ngOnInit(): void
  {
    ...
    this.getAllCustodians(currentPage, pageSize)
    this.actionCellService.primaryClicked$.pipe(takeUntil(this.isDestroyed$)).subscribe((data) =>
    {
      if (data.gridName === GridType.Custodians)
      {
        this.onEditClick(data.rowData.custodianID)
      }
    })
    this.actionCellService.secondaryClicked$.pipe(takeUntil(this.isDestroyed$)).subscribe((data) =>
    {
      if (data.gridName === GridType.Custodians)
      {
        this.removeHandler(data.rowData.custodianID)
      }
    })
  ...
  private searchHandler()
  {
    ...
    ).subscribe((term) =>
    {
      ...
      this.getAllCustodians(1, this.pagination.pageSize);
    });
  }
  ...
  getAllCustodians(currentPage: number, pageSize: number) {
    ...
    this.tableDataService
      .getTableDataWithPagination('Custodians/GetAllCustodians', payload)
      .pipe(
        first(),
  ...
  removeHandler(custodianID: number) {
    this.confirmationDialogService.confirm().then((confirmed) => {
      if (confirmed) {
        this.sendingRequest = true;
        const payload = { custodianID };
        this.tableDataService
          .getTableData('Custodians/DeleteCustodian', { delete: 1, ...payload })
          .pipe(
            first(),
            finalize(() => (this.fetchingData = false))
          )
          .subscribe({
            next: (res) => {
              if (res && res.status === '200') {
                ...
                this.getAllCustodians(this.pagination.currentPage, this.pagination.pageSize);
              } else {
  ...
  exportToCSV(): void
  {
    this.gridDataService.exportToCSV(this.gridApi, GridType.Custodians);
  }
}
