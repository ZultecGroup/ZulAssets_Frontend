...
import { ColDef, GridApi, GridReadyEvent, ProcessCellForExportParams, ProcessHeaderForExportParams } from 'ag-grid-community';
...
import { AddressTemplateDto, AddressTemplateDtoResponse } from '../../shared/dtos/AddressTemplate/AddressTemplateDto';

@Component({
  selector: 'app-address-templates',
  templateUrl: './address-templates.component.html',
  styleUrls: ['./address-templates.component.scss'],
})
export class AddressTemplatesComponent implements OnInit {
  gridData: AddressTemplateDto[] = [];
  gridView: AddressTemplateDto[] = [];
  fetchingData: boolean = false;
  sendingRequest: boolean = false;
  searchText: string = '';
  searchSubject = new Subject<string>();
  addressTemplateGridCols: ColDef[] = [];
  ...
  constructor(
    ...
    this.addressTemplateGridCols = this.gridDataService.getColumnDefs(GridType.AddressTemplate, this.GeneralService.permissions['Address Templates']);
  }
  ...
  ngOnInit(): void {
    ...
    this.actionCellService.primaryClicked$.pipe(takeUntil(this.isDestroyed$)).subscribe((data) => {
      if (data.gridName === GridType.AddressTemplate) {
        this.onEditClick(data.rowData.addressID)
      }
    })
    this.actionCellService.secondaryClicked$.pipe(takeUntil(this.isDestroyed$)).subscribe((data) => {
      if (data.gridName === GridType.AddressTemplate) {
        this.removeHandler(data.rowData.addressID)
      }
    })
  ...
  getAllAddresses(currentPage: number, pageSize: number) {
    ...
    this.tableDataService
      .getTableDataWithPagination('AddressTemplate/GetAllAddressTemplates', payload)
      .pipe(
        first(),
        finalize(() => (this.fetchingData = false))
      )
      .subscribe({
        next: (res: AddressTemplateDtoResponse) => {
          if (res) {
  ...
  exportToCSV(): void {
    this.gridDataService.exportToCSV(this.gridApi, GridType.AddressTemplate);
  }
